...
function doInvokeWebkitBridgeCall(function_name, args, id) {
    if (!window.webkit) {
        throw new Error("window.webkit is not defined, can't call " + function_name);
    }
    return window.webkit.messageHandlers.fabricJsBridge
    .postMessage(
        JSON.stringify(
            [function_name, args, id]
        )
    );
}
...
window.fabric = {
    ...
    createBridge: (func_list) => {
        for (var i = 0; i < func_list.length; i++) {
            ...
            var nested_object = func_hierarchy.reduce(function (obj,
                prop) {
                if (!obj[prop]) {
                    obj[prop] = {};
                }
            ...
            var func_body =
                ...
                '    window.fabric.checkReturnValue("' + func_name +
                '", resolve, reject, __id__);' +
                '});' +
                'doInvokeWebkitBridgeCall("' + func_name +
                '", arguments, __id__);' +
                'return promise;';
    ...
    checkReturnValue: (func_name, resolve, reject, id) => {
        var check = setInterval(function () {
            var return_obj = window.fabric._pool[
                func_name][id];
            if (return_obj) {
                var value = return_obj.value;
                var error = return_obj.error;

                delete window.fabric._pool[func_name][
                    id
                ];
                clearInterval(check);
